...
const PROJECT_NAME = "PantrySwipe";

const PRODUCT_IDENTIFIER = "premium_monthly";
const PLAY_STORE_PRODUCT_IDENTIFIER = "premium_monthly:monthly";

const PRODUCT_DISPLAY_NAME = "PantrySwipe Premium";
const PRODUCT_USER_FACING_TITLE = "PantrySwipe Premium";
const PRODUCT_DURATION = "P1M";

const APP_STORE_APP_NAME = "PantrySwipe iOS";
const APP_STORE_BUNDLE_ID = "com.pantryswipe.app";
const PLAY_STORE_APP_NAME = "PantrySwipe Android";
const PLAY_STORE_PACKAGE_NAME = "com.pantryswipe.app";
...
const PACKAGE_DISPLAY_NAME = "Monthly Premium";

// S$4.99/month  (test store prices are immutable once set)
const PRODUCT_PRICES = [
  { amount_micros: 4990000, currency: "SGD" }, // S$4.99
  { amount_micros: 3990000, currency: "USD" }, // US$3.99
];
...
async function seedRevenueCat() {
  ...
  // ── API Keys ─────────────────────────────────────────────────────────────────
  const { data: testStoreApiKeys, error: testApiKeysError } = await listAppPublicApiKeys({
    client,
    path: { project_id: project.id, app_id: app.id },
  });
  if (testApiKeysError) throw new Error("Failed to get Test Store API keys");

  const { data: appStoreApiKeys, error: appApiKeysError } = await listAppPublicApiKeys({
    client,
    path: { project_id: project.id, app_id: appStoreApp.id },
  });
  if (appApiKeysError) throw new Error("Failed to get App Store API keys");

  const { data: playStoreApiKeys, error: playApiKeysError } = await listAppPublicApiKeys({
    client,
    path: { project_id: project.id, app_id: playStoreApp.id },
  });
  if (playApiKeysError) throw new Error("Failed to get Play Store API keys");
  ...
  console.log("====================\n");
  console.log("Next: copy the keys above into these environment variables:");
